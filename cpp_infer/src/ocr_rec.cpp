...
namespace PaddleOCR {
    ...
    void CRNNRecognizer::Run(std::vector<std::vector<std::vector<int>>> boxes,
        cv::Mat& img, Classifier* cls, std::vector<std::string>& out_str, std::vector<float>& out_score) {
        cv::Mat srcimg;
        img.copyTo(srcimg);
        cv::Mat crop_img;
        cv::Mat resize_img;
        ...
        for (int i = 0; i < boxes.size(); i++) { // 遍历每一个文字盒 
            ...
            for (int n = 0; n < predict_shape[1]; n++) {
                ...
                if (argmax_idx > 0 && (!(n > 0 && argmax_idx == last_index))) {
                    ...
                    strget += label_list_[argmax_idx]; // char添加到str末尾 
                }
